...
class StateTest(unittest.TestCase):
    ...
    def test_is_pending(self) -> None:
        self.assertTrue(SyncState('PENDING', datetime.now(), 'link')
                        .is_pending)

    def test_is_outdated(self) -> None:
        self.assertTrue(SyncState('OUTDATED', datetime.now(), 'link')
                        .is_outdated)

    def test_is_init(self) -> None:
        self.assertTrue(SyncState('INITIALIZING', datetime.now(), 'link')
                        .is_init)
