...
from pymultimatic.api import urls, Connector
...
def mock_auth(resp_mock: aioresponses) -> None:
    resp_mock.post(urls.new_token(), status=200,
...
def _room() -> Room:
    ...
    return Room(id='id', name='name', time_program=timeprogram,
                target_high=20, temperature=20,
                operating_mode=OperatingModes.AUTO)


def _hotwater() -> HotWater:
    timeprogram = _time_program()
    return HotWater(id='dhw', name="HotWater", temperature=38,
                    time_program=timeprogram, target_high=50,
                    operating_mode=OperatingModes.AUTO)


def _zone() -> Zone:
    timeprogram = _time_program(SettingModes.DAY)
    heating = ZoneHeating(time_program=timeprogram,
                          operating_mode=OperatingModes.AUTO,
                          target_high=25, target_low=22)
...
def _circulation() -> Circulation:
    timeprogram = _time_program()
    return Circulation(id='dhw', name="Circulation",
